...
@section('conteudo')
    ...
    <form action="{{route('bairros.update', $bairro->id)}}" method="post">
        @csrf
        <h1 class="gray-900 text-2xl font-bold">Editando bairro: {{$bairro->nome}}</h1>
        <div class="w-[400px] mt-6 max-[700px]:w-[90%]">
            <label for="nome" class="font-bold">Nome do bairro</label>
            <br>
            <input type="text" name="nome" id="" placeholder="Digite seu email" class="bg-gray-300 rounded-md py-2 px-1 text-sm w-full my-2 focus:shadow-md focus:outline-[#97D776]" value="{{$bairro->nome}}">
    ...
    <form action="{{route("bairros.destroy", $bairro->id)}}" method="post" class="mt-1">
        @csrf
        @method("delete")
        <button
