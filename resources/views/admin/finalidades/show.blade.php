...
@section('conteudo')
    ...
    <form action="{{route('finalidades.update', $finalidade->id)}}" method="post">
        @csrf
        <h1 class="gray-900 text-2xl font-bold">Editando finalidade: {{$finalidade->nome}}</h1>
        <div class="w-[400px] mt-6 max-[700px]:w-[90%]">
            <label for="nome" class="font-bold">Nome da finalidade</label>
            <br>
            <input type="text" name="nome" id="" placeholder="Digite seu email" class="bg-gray-300 rounded-md py-2 px-1 text-sm w-full my-2 focus:shadow-md focus:outline-[#97D776]" value="{{$finalidade->nome}}">
    ...
    <form action="{{route("finalidades.destroy", $finalidade->id)}}" method="post" class="mt-1">
        @csrf
        @method("delete")
        <button
